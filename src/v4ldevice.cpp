...
/* From e-consystems*/
/*------------------------------------------------------*/
		if(0 > posix_memalign(&buffers[n_buffers].start,getpagesize(),buffer_size) )
			exit(EXIT_FAILURE);
...
//from e-consystems
void helper_q_cam_frame(void)
{
	if (-1 == xioctl(fd, VIDIOC_QBUF, &buf))
		errno_exit("VIDIOC_QBUF");
}

int helper_dq_cam_frame(char **pointer_to_cam_data, int *size)
{
	unsigned int count;
	...
	while (count-- > 0) {
		for (;;) {
			...
			//*pointer_to_cam_data = buffers[buf.index].start;
			//*size = buf.bytesused;
			break;
			/* EAGAIN - continue select loop. */
		}
